...
export default function Add() {
  ...
  const submitHanlder = async (e) => {
    ...
    if (result) {
      ...
      const obj = {
        ...
        goals: selectedGoals.map(g => g.value)
      };
  ...
  const fetchDetails = async () => {
    ...
    if (response?.status) {
      ...
      const pre = response.data.goals.map((g) => ({
        value: g._id,
        label: g.title,
      }));
      setSelectedGoals(pre);
  ...
  return (
    <div className="w-full">
      ...
      {data && (
        <div className="w-1/2 xs:w-full sm:w-full md:w-2/3 lg:w-2/3  success pb-8">
          ...
          <ImageUploader
            ...
            fileAdded={thumbnail?.url}
            keyUrl={thumbnail?.key}
            uploaded={true}
          />
          <h6 className="text-[#252322] font-semibold mt-5 text-sm mb-1">
            Title
            ...
            !showContent && (
              <div
                onClick={(e) => {
                  setShowContent(true);
                  toast.info(
                    "Content will save directly after clicking on save",
                    {
                      toastId: "dkjhfgdh",
                    }
                  );
          ...
          {contentTab == "course" && contentArray.length > 0 && !edited && (
            <div className="w-full bg-white rounded-xl p-5 xs:p-2">
              {contentArray.length > 0 &&
                contentArray?.map((item, index) => (
                  <SingleContentAdded
                    ...
                    edit={true}
                    length={contentArray.length > 1 ? true : false}
                    index={index}
                  />
                ))}
            </div>
            ...
            <div className="w-full">
              ...
              {edited && (
                <AudioVideoUploader
                  ...
                  contentType={courseContentType}
                  uploaded={true}
                  fileAdded={contentFile?.url}
                  keyUrl={contentFile?.key}
                  callback={conteFileHandler}
                />
              )}
