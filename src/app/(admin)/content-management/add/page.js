...
export default function Add() {
  ...
  const updateHandler = (obj) => {
    ...
    setContentThumbnail(
      obj?.thumbnail == "string"
        ? obj?.thumbnail
        : URL.createObjectURL(obj?.thumbnail)
    );
    setContentFile(
      obj?.media == "string" ? obj?.media : URL.createObjectURL(obj?.media)
    );
  ...
  const submitHanlder = async (e) => {
    ...
    if (result) {
      if (contentTab == "single") {
        ...
        if (response?.status) {
          temp.map((item) => {
            addChapetrInCourseHandler(item, response.data._id);
          });
          setTimeout(() => {
            router.push("/content-management");
          }, [200]);
  ...
  const addChapetrInCourseHandler = async (obj, id) => {
    const formdata = new FormData();
