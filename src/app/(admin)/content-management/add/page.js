...
import {
  addChapterInCourse,
  addChapterInCourseTwo,
  createChapter,
  createCourse,
  createCourseTwo,
  createUnitaryCourse,
  createUnitaryCourseNew,
  getAllCategories,
  getAllChapters,
  ...
  const submitHanlder = async (e) => {
    ...
    if (result) {
      if (contentTab == "single") {
        ...
        if (response?.status) {
          temp.map((item, index) => {
            addChapetrInCourseHandler(
              item,
              response.data._id,
              index + 1 == temp.length ? true : false,
              index
            );
          });
        } else {
      ...
      <div className="w-1/2 xs:w-full sm:w-full md:w-2/3 lg:w-2/3 success">
        ...
        <div className="grid grid-cols-2 p-1 bg-[#DADDF1] rounded-[80px] mt-12">
          <h6
            ...
            className={`text-sm p-2 text-center rounded-[80px] cursor-pointer ${
              contentTab == "course"
                ? "font-semibold text-[#00] bg-white"
                : "font-normal text-[#818181] bg-transparent "
            } `}
          >
            Course
          </h6>
        ...
        />
        {contentTab == "course" && contentArray.length > 0 && !edited && (
          <h6 className="text-[#252322] font-semibold mt-5 text-sm mb-1">
            Content
          </h6>
        )}
        {contentTab == "course" && contentArray.length == 0 && !showContent && (
          <div
            onClick={(e) => setShowContent(true)}
        ...
        {contentTab == "course" &&
          ...
          !showContent && (
            ...
            >
              ...
              <h5 className="text-[#3090E9] text-sm font-normal">
                Add content
              </h5>
            </div>
          )}
        ...
        <div className="flex items-center gap-5">
          {!showContent && disableButton && (
            <button className="p-4 text-white font-black mt-5 bg-[#AE445A] opacity-30 rounded-xl w-3/12 flex justify-center">
          ...
          {!showContent && (
            <Link href="/content-management" className="w-3/12">
              <button className="p-4 text-[#AE445A] mt-5 font-black border border-[#AE445A] rounded-xl flex  justify-center">
                Cancel
              </button>
            </Link>
          )}
        ...
        {showContent && contentTab == "course" && (
          <div className="w-full">
            <h6 className="text-[#252322] font-semibold mt-5 text-sm mb-1">
              {edited ? "Edit" : "Add"} content to course
            </h6>
            <h6 className="text-[#252322] font-semibold mt-5 text-sm mb-1">
              Thumbnail
            </h6>
              ...
              <ImageUploader
                ...
                fileAdded={
                  contentThumbnail == "string"
                    ? contentThumbnail?.url
                    : contentThumbnail?.url
                }
                callback={thumbnailContentHandler}
              />
            )}
            <h6 className="text-[#252322] font-semibold mt-5 text-sm mb-1">
              Title
            ...
            <div className="grid grid-cols-2 gap-5 mt-1">
              ...
              >
                ...
                {contentAccessType == "free" ? (
                  <input
                    ...
                    checked
                    value="free"
                    onChange={accessibilityContentTabHandler}
                  />
                ) : (
                  <input
                    type="radio"
              ...
              {accessibilityTab == "free" && (
                <label
                  htmlFor="premiumC"
                  className={`bg-white opacity-35 cursor-pointer flex items-center justify-between border-2 border-solid  rounded-xl px-4 py-3 ${
                    contentAccessType != "premium"
                      ? "border-[#E7E5E4]"
                      : "border-[#08A03C]"
                  }`}
            ...
            <div className="mt-5 flex flex-wrap gap-5">
              {!disableButton && (
                <div
                  onClick={contentAddMoreHandler}
                  className="bg-[#3090E920] w-fit cursor-pointer rounded-3xl p-4 flex items-center gap-2"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
                  ...
                  <h5 className="text-[#3090E9] text-sm font-normal">
                    Save & Add more lessons
                  </h5>
                </div>
              )}
              {disableButton && (
                <div className="bg-[#3090E920] opacity-50 w-fit cursor-pointer rounded-3xl p-4 flex items-center gap-2">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
                    height="20"
              ...
              <button
                ...
                className="p-4 text-[#AE445A] font-black border border-[#AE445A] rounded-xl flex w-3/12 justify-center"
              >
                Cancel
              </button>
              {!disableButton && (
                <button
                  onClick={contentSaveHandler}
                  className="p-4 text-white font-black bg-[#AE445A] rounded-xl flex w-3/12 justify-center"
                >
                  Save
                </button>
              )}
              {disableButton && (
                <button className="p-4 text-white font-black bg-[#AE445A] opacity-30 rounded-xl flex w-3/12 justify-center">
                  Save
                </button>
              )}
            </div>
