...
function Page({ params }) {
  ...
  const fetchData = async (page) => {
    setLoading(true);
    setData([])
    const result = await getWatchedVideo(users, page, sort, searchTerm);
    if (result.status) {
    ...
    <div className="flex flex-col gap-7">
      <div className="flex flex-row gap-5 items-center">
        ...
        <p className="text-userblack font-semibold text-xl2 font-sans">
          Users management -{" "}
          <span className="text-[#AE445A]"> Video watched</span>{" "}
        </p>
      </div>
      {/* <UserDetailsBox/> */}
          ...
          <div className="flex flex-col bg-white min-w-fit w-full">
            {data &&
              data.map((item, index) => (
                <div
                  key={item._id || index}
                  className=" grid grid-cols-userVideoTable justify-between border-b border-[#E9E9EC] items-center p-4"
                >
                  ...
                  <div className="text-userblack font-sans w-[350px]  gap-2 font-normal text-base">
                    <p>
                      {truncateDescription(item.chapter?.description) ||
                        "--Na--"}
                    </p>
                  </div>
                  <span className="text-userblack font-sans font-normal  text-base">
                    {item.chapterCategory?.title}
                  </span>
                  <span className="text-userblack font-sans font-normal  text-base">
                    {dayjs(item.createdAt).format("DD/MM/YYYY")}
                  </span>
                  <span className="text-userblack font-sans font-normal  text-base">
                    {truncateName(item.chapterCreator?.firstName) || "NA"}
                  </span>
