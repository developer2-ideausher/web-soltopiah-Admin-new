...
export default function AudioVideoUploader(props) {
  ...
  //   const calculateDuration = (fileType, fileUrl, url, filekey) => {
  ...
  const calculateDuration = (fileType, fileUrl, url, filekey) => {
    ...
    media.addEventListener("loadedmetadata", () => {
      ...
      // Numeric format like 4.50 instead of 4.83
      ...
      // console.log(formattedDuration, numericDuration, typeof numericDuration); // "4:50" 4.5
  ...
  return (
    <div
      className={`bg-white p-2 rounded-xl flex flex-col items-center justify-center w-full`}
    >
      {!isUploaded && !loading && (
        <div
      ...
      {loading && (
        <h5 className="flex items-center justify-center text-3xl text-primary font-bold h-52">
          {progress}%
        </h5>
      )}
      ...
      {isUploaded && !loading && (
        <div
          ...
          className="flex  w-full cursor-pointer justify-center items-center bg-[#feecec] mt-4 gap-2 p-2 rounded-full"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
