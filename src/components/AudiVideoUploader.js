...
export default function AudioVideoUploader(props) {
        ...
        if (e.target.files[0]) {
            ...
            if (isVideo) {
                if (acceptType !== 'audio') {
                    ...
                    toast.info('You can only upload audio files as the first content was audio.', {
            ...
            // Handle audio files
                ...
                if (acceptType !== 'video') {
                    ...
                    toast.info('You can only upload video files as the first content was video.', {
    ...
    const calculateDuration = (fileType,fileUrl,url,filekey) => {
        ...
        media.addEventListener('loadedmetadata', () => {
            ...
            props.callback(spiltUrl[0],filekey,durationInMinutes.toFixed(2), fileType.startsWith('video') ? 'video' : 'audio');
        });
    };
    ...
    return (
        <div className={`bg-white p-2 rounded-xl flex flex-col items-center justify-center w-full`}>
            ...
            {isUploaded && !loading && (
                <div onClick={removeHandler} className="flex  w-full cursor-pointer justify-center items-center bg-[#feecec] mt-4 gap-2 p-2 rounded-full">
