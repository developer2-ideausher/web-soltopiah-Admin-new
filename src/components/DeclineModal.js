...
function DeclineModal({ onclose, onSubmitClick }) {
  ...
  const handleSubmit = (event) => {
    ...
    // Pass the explanation to the parent component
    onSubmitClick(explanation);
    // router.push("/live-manage/live-requests");
  };
  return (
    <form
      onSubmit={handleSubmit}
      className="flex justify-center items-center bg-white w-[700px] max-h-[80vh] rounded-lg"
    >
      <div className="flex flex-col items-center gap-8 w-full ">
        ...
        <div className="flex flex-col items-start w-full py-3 px-4 gap-2">
          <label
            className="text-sm font-sans font-semibold text-[#414554]"
            htmlFor="Explain"
          >
            Briefly explain the reason
          </label>
          <textarea
            className="py-3 px-4 border border-[#E7E5E4] rounded-xl w-full"
            value={explanation}
        ...
        <div className="py-3 px-4 w-full">
          <button
            type="submit"
            className="w-full text-base font-sans font-normal text-white p-3 bg-[#EE3E3E] border border-[#EE3E3E] rounded-md "
          >
            Submit
          </button>
        </div>
