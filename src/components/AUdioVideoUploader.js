...
export default function AudioVideoUploader(props) {
    ...
    const calculateDuration = (fileUrl, fileType) => {
        ...
        media.addEventListener('loadedmetadata', () => {
            ...
            props.callback(file, durationInMinutes.toFixed(2), fileType.startsWith('video') ? 'video' : 'audio');
        });
    };
    ...
    return (
        <div className={`${isUploaded && 'bg-white p-2 rounded-xl'}`}>
            ...
            {isUploaded && (
                <div onClick={removeHandler} className="flex cursor-pointer justify-center items-center bg-[#feecec] mt-4 gap-2 p-2 rounded-full">
